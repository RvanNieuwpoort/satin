...
public class StoreLoadPath extends Path {
    ...
    StoreLoadPath(InstructionHandle storeInstruction, BasicBlock block,
	    LocalVariableGen[] localVariableIndices) {
	super(new Path());
	add(block);
    ...
    StoreLoadPath(InstructionHandle storeInstruction, Path path,
	    LocalVariableGen[] localVariableIndices) throws NeverReadException {
	super(path);
	...
	try {
	    int indexEarliestBasicBlock = getIndexEarliestBasicBlock(
		    this.localVariableIndices, this.storeInstruction);
     ...
     */
    private int getIndexPauseBasicBlock(InstructionHandle storeInstruction)
    ...
    private boolean containsLoadWithIndex(LoadAwareBasicBlock basicBlock,
	    LocalVariableGen[] localVariableIndices) {
    ...
    private boolean containsLoadWithIndexAfter(LoadAwareBasicBlock basicBlock,
	    LocalVariableGen[] localVariableIndices,
	    InstructionHandle instruction) {
	for (LocalVariableGen localVariableIndex : localVariableIndices) {
	    if (basicBlock.containsLoadWithIndexAfter(instruction,
		    localVariableIndex)) {
		return true;
	    }
	}
     ...
     */
    private int getIndexEarliestBasicBlock(
	    LocalVariableGen[] localVariableIndices,
	    InstructionHandle storeInstruction) throws NeverReadException {
	for (int i = 0; i < size(); i++) {
	    LoadAwareBasicBlock basicBlock = new LoadAwareBasicBlock(get(i));
	    if (i == 0) {
		if (containsLoadWithIndexAfter(basicBlock,
			localVariableIndices, storeInstruction)) {
		    return i;
		}
	    } else {
