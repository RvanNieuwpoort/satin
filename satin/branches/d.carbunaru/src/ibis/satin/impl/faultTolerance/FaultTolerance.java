...
public final class FaultTolerance implements Config {
	...
	ftComm = new FTCommunication(s); // must be created first, it handles
					 // registry upcalls.
    }
	...
	if (CHECKPOINTING) {
		...
		cpThread = new CheckpointThread(CHECKPOINT_INTERVAL,
			CHECKPOINT_FIRST);
		cpThread.start();
	    }
	...
	try {
		...
		if (id.equals(s.getMasterIdent())
			|| id.equals(clusterCoordinatorIdent)) {
		    try {
	...
	if (s.isMaster()) {
	    ArrayList<InvocationRecord> toStore = s.onStack.killChildrenOf(
		    targetStamp, true);
	...
	for (ClientThread ct : s.clientThreads) {
	    ArrayList<InvocationRecord> toStore = ct.onStack.killChildrenOf(
		    targetStamp, true);
	...
	if (s.isMaster()) {
	    ArrayList<InvocationRecord> toStore = s.onStack
		    .killSubtreesOf(targetOwner);
	...
	for (ClientThread ct : s.clientThreads) {
	    ArrayList<InvocationRecord> toStore = ct.onStack
		    .killSubtreesOf(targetOwner);
	...
	synchronized (s) {
	    if (s.isMaster()) {
		ArrayList<InvocationRecord> tmp = s.onStack
			.getAllFinishedChildren(v);
	    ...
	    for (ClientThread ct : s.clientThreads) {
		ArrayList<InvocationRecord> tmp = ct.onStack
			.getAllFinishedChildren(v);
    ...
    /*
     * public void handleDelayedMessages() { if (gotCrashes) {
     * s.ft.handleCrashes(); } if (gotAbortsAndStores) {
     * s.ft.handleAbortsAndStores(); } if (gotDelete) { s.ft.handleDelete(); }
     * if (gotDeleteCluster) { s.ft.handleDeleteCluster(); } if
     * (masterHasCrashed) { s.ft.handleMasterCrash(); } if
     * (clusterCoordinatorHasCrashed) { s.ft.handleClusterCoordinatorCrash(); }
     * if (updatesToSend) { globalResultTable.sendUpdates(); } if
     * (CHECKPOINTING) { if (gotCheckpoints){ synchronized (s){
     * s.stats.writeCheckpointTimer.start(); try {
     * checkpointFile.write(checkpoints); } finally {
     * s.stats.writeCheckpointTimer.stop(); } } gotCheckpoints = false; } if
     * (becomeCoordinator){ coordinatorInit(); becomeCoordinator = false; } if
     * (findNewCoordinator){ findNewCoordinator(); findNewCoordinator = false; }
     * if (setCoordinator){ setCoordinator(); setCoordinator = false; } if
     * (takeCheckpoint){ // if (coordinator && !CHECKPOINT_PUSH){ //
     * broadcastCheckpointRequest(); // takeCoordinatorCheckpoint(); // } else
     * if (coordinator && CHECKPOINT_PUSH){ // takeCoordinatorCheckpoint(); // }
     * else if (!coordinator && CHECKPOINT_PUSH){ // takeAndSendCheckpoint(); //
     * } if (coordinator) { if (!CHECKPOINT_PUSH) {
     * broadcastCheckpointRequest(); } takeCoordinatorCheckpoint(); } else { if
     * (CHECKPOINT_PUSH) { takeAndSendCheckpoint(); } } takeCheckpoint = false;
     * } if (gotCheckpointAndQuit) { handleCheckpointAndQuit(); } }
     * 
     * }
     */
	...
	if (CHECKPOINTING) {
		 ...
		 * broadcastCheckpointRequest(); takeCoordinatorCheckpoint(); }
		 * else if (coordinator && CHECKPOINT_PUSH){
		 * takeCoordinatorCheckpoint(); } else if (!coordinator &&
		 * CHECKPOINT_PUSH){ takeAndSendCheckpoint(); }
		 */
		if (coordinator) {
		    if (!CHECKPOINT_PUSH) {
	...
	synchronized (s) {
	    if (coordinator) {
		takeCoordinatorCheckpoint();
		System.out.println("SATIN '" + s.ident + "': checkpoint taken");
		...
		System.out
			.println("SATIN '" + s.ident + "': coordinator quits");
	    } else {
		takeAndSendCheckpoint();
		System.out.println("SATIN '" + s.ident + "': checkpoint taken");
	    }
    ...
    /**
     * Makes the tempCoordinator (i.e. the node which has the fastes filewrite
     * time uptill now) the coordinator and lets the other nodes know about the
     * new coordinator
     **/
    ...
    /**
     * Makes this node coordinator: - start the necessary threads - initialize
     * the checkpoint file - broadcast checkpoint info to other nodes
     **/
	...
	if (parent != null && parent.isReDone() || parent == null && restarted) {
		...
		ftLogger.debug("Parent is redone, setting redone of job "
			+ r.getStamp());
	    }
	    r.setReDone(true);
	}
