 ...
 * @author rob
 * 
 *         A Victim represents an Ibis we can steal work from. This class is
 *         immutable, only the sendport itself could be connected and
 *         disconnected.
 * 
 */
public final class Victim implements Config {
	...
	this.sendPort = s;
	if (s != null) {
	    inDifferentCluster = !clusterOf(ident).equals(
		    clusterOf(s.identifier().ibisIdentifier()));
	} else {
	...
	if (connected) {
		...
		commLogger.debug("SATIN '"
			+ sendPort.identifier().ibisIdentifier()
	...
	synchronized (sendPort) {
		...
		// ignore
		Config.commLogger.warn("SATIN '"
			+ sendPort.identifier().ibisIdentifier()
			+ "': port.close() throws exception (ignored), dest = "
			+ ident, e);
	    }
	}
    }
