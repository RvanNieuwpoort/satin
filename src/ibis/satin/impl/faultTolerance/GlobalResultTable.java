...
public final class GlobalResultTable implements Config, Protocol {
    ...
    /**
     * A list of updates that has to be broadcast to the other nodes. Elements
     * are of type (Stamp, GlobalResultTableValue).
     */
	...
	try {
		...
		if (grtLogger.isDebugEnabled()) {
		    grtLogger.debug("SATIN '" + s.ident + "': lookup failed "
			    + key);
		}
	    }
	...
	try {
		...
		try {
		    ...
		    s.stats.tableSerializationTimer
		...
		try {
		    ...
		    grtLogger.debug("SATIN '" + s.ident + "': " + msgSize
			    + " sent in "
			    + s.stats.tableSerializationTimer.lastTimeVal()
			    + " to " + v);
		} catch (IOException e) {
		    m.finish(e);
		    grtLogger.info("SATIN '" + s.ident
			    + "': Got exception in finish()");
		    // always happens after a crash
		}
	    }
	...
	for (Map.Entry<Stamp, GlobalResultTableValue> entry : entries
		.entrySet()) {
	...
	try {
	    ...
	    Map<Stamp, GlobalResultTableValue> map = (Map<Stamp, GlobalResultTableValue>) m
		    .readObject();
	    synchronized (s) {
		addContents(map);
	    }
