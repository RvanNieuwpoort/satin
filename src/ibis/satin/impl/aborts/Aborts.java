...
public final class Aborts implements Config {
	...
	try {
	    ...
	    if (inletLogger.isDebugEnabled()) {
		inletLogger
			.debug("Got an exception from exception handler! " + t
				+ ", r = " + r + ", r.parent = "
				+ r.getParent(), t);
	    }
	...
	synchronized (r) {
		...
		throw new Error(
			"Spawned job threw exception. Invocation record = " + r,
			r.eek);
	    }
	    if (ASSERTS && r.getParentLocals() != null) {
		if (ct == null) {
		    s.assertFailed("parentLocals is not null in empty inlet",
			    new Exception());
		} else {
		    ct.assertFailed("parentLocals is not null in empty inlet",
			    new Exception());
		}
	    }
