...
public final class ClusterAwareRandomWorkStealing extends
	...
	// First look if there was an outstanding WAN steal request that
	// resulted
	// in a job.
	...
	if (job != null) {
		...
		if (ct == null) {
		    stealLogger.info("Executing intercluster job "
			    + job.getStamp());
		} else {
	...
	if (clientThread == null) {
		...
		if (FT_NAIVE || canDoAsync) {
			...
			commLogger.warn("SATIN '" + s.ident
				+ "': Got exception during wa steal request: "
				+ e);
			// Ignore this?
		    }
		}
		...
		if (FT_NAIVE || canDoAsync) {
		    ...
		    try {
			asyncStealStart = System.currentTimeMillis();
			clientThread.lb.sendStealRequest(remoteVictim, false,
				false);
		    } catch (IOException e) {
			commLogger.warn("SATIN '" + s.ident + ", Thread "
				+ ct.id
				+ "': Got exception during wa steal request: "
				+ e);
			// Ignore this?
	...
	if (asyncStealInProgress) {
	    if (clientThread == null) {
		stealLogger
			.info("waiting for a pending async steal reply from "
				+ asyncCurrentVictim);
		synchronized (satin) {
		    while (asyncStealInProgress && !gotAsyncStealReply) {
			try {
			    satin.handleDelayedMessages(); // TODO move outside
							   // lock --Rob
			    satin.wait(250);
		...
		stealLogger.info("Thread " + clientThread.id
			+ ": waiting for a pending async steal reply from "
			+ asyncCurrentVictim);
		synchronized (satin) {
		    while (asyncStealInProgress && !gotAsyncStealReply) {
			try {
			    clientThread.handleDelayedMessages(); // TODO move
								  // outside
								  // lock --Rob
     ...
     * if the owner of the asynchronously stolen job crashed, abort the job.
     */
	...
	synchronized (satin) {
		...
		s.clientThreads[threadId].algorithm.asyncJobResultWorkerThread(
			ir, sender);
	    } else {
		if (stealLogger.isInfoEnabled() && ir != null) {
