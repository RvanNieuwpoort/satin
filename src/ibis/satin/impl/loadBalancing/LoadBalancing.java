...
public final class LoadBalancing implements Config {
	...
	if (ct == null) {
	    ...
	    int threadId;
	    synchronized (s) {
		// there is always at least 1 thread id in the list in the map:
		// <ibisident, list<threadid>>.
	...
	if (Satin.GLOBAL_PAUSE_RESUME) {
		...
		if (s.comm.paused) {
			...
			commLogger.info("SATIN '" + s.ident + "', Thread "
				+ ct.id + ": paused for " + (end - start)
				+ " ms");
		    }
		}
	    }
	...
	while (true) {
		...
		if (ct == null) {
			...
			ct.currentVictimCrashed = false;
			ftLogger.debug("SATIN '" + s.ident + "', Thread "
				+ ct.id + "': current victim crashed");
			return;
		    }
		}
