...
final class MethodTable {
	...
	boolean[] analyseUsedLocals(MethodGen mg, CodeExceptionGen catchBlock,
		...
		if (curr instanceof BranchInstruction) {
		    ...
		    if (dest.getPosition() < catchBlock.getHandlerPC()
			    .getPosition() || // backjump out of handler
	...
	for (int i = 0; i < locals.length; i++) {
		...
		types[maxpos] = t;
		System.out.println("rewriting local from index " + ind + " to "
			+ maxpos);
	...
	for (int i = 0; i < methods.length; i++) {
	    ...
	    mg.setMaxLocals();
	    System.out.println("Method now has " + mg.getMaxLocals()
		    + " locals");
	    mg.setMaxStack();
	...
	return name
		+ "_"
		+ type.toString().replace('.', '_').replace('[', '_')
			.replace(']', '_').replace('$', '_');
    }
