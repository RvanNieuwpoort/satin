...
public final class Satinc extends IbiscComponent {
	...
	StoreClass(InstructionList store, Method target, JavaClass cl) {
		...
		classNameNoPackage = className.substring(
	...
	MethodGen new_main = new MethodGen(Constants.ACC_STATIC
		...
		Type.STRING, 1) }, new String[] { "argv" }, "main",
	...
	InstructionHandle try_start = il.append(ins_f.createInvoke(
		clg.getClassName(), origMain.getName(), Type.VOID,
	...
	il.append(ins_f.createInvoke("ibis.satin.impl.Satin", "exit",
		Type.VOID,
		new Type[] { new ObjectType("java.lang.Throwable") },
	...
	return getFileBase(packageName,
		classNameNoPackage + "_" + do_mangle(m), "Satin_",
		"_LocalRecord");
    }
	...
	il.insert(pos, ins_f.createFieldAccess(
		"ibis.satin.impl.spawnSync.InvocationRecord", "cacheNext",
		irType, Constants.GETFIELD));
	...
	for (int k = 0; k < idTable.size(); k++) {
		...
		il.insert(pos, ins_f.createFieldAccess(
			"ibis.satin.impl.spawnSync.InvocationRecord",
			"storeId", Type.INT, Constants.GETFIELD));
		...
		if (isArrayStore(storeIns.getStart())) {
		    ...
		    il.insert(pos, ins_f.createFieldAccess(invClass, "array",
		    ...
		    il.insert(pos,
			    ins_f.createCheckCast(pf.getReferenceType(cpg)));
		    il.insert(pos, new ALOAD(maxLocals + 3));
		}
	    ...
	    il.insert(pos, ins_f.createInvoke(
		    ...
		    new Type[] { new ObjectType(invocationRecordName(
			    ...
			    getStorePackage(k))) }, Constants.INVOKESTATIC));
	...
	retval = il.insert(pos,
		ins_f.createNew("ibis.satin.impl.aborts.AbortException"));
	il.insert(pos, new DUP());
	il.insert(pos, ins_f.createInvoke(
	...
	if (!returnType.equals(Type.VOID)) {
		...
		System.err
			.println("Error: \"return <spawnable method>\" is not "
		...
		System.err
			.println("Error: \"not storing the result of a <spawnable method>\" is not "
				+ "allowed, class "
				+ m.getClassName()
				+ ", line " + l);
		System.exit(1);
	...
	ih = il.append(ih, ins_f.createInvoke(
		invocationRecordName(target, clname, pnam), methodName,
		new ObjectType(invocationRecordName(target, clname, pnam)),
		parameters, Constants.INVOKESTATIC));
	...
	String s = (cls instanceof ObjectType) ? ((ObjectType) cls)
	...
	while (cl != null) {
		...
		System.out.println("findMethod: could not find class "
	...
	while (cl != null) {
		...
		System.out.println("findMethodClass: could not find class "
			+ cl.getSuperclassName());
		return null;
	    }
	}
	System.out.println("findMethodClass: could not find method " + name
		+ sig);
	return null;
    }
	...
	m.addLocalVariable("local_record", new ObjectType(
		"ibis.satin.impl.aborts.LocalRecord"), maxLocals + 3, lr_h,
		null);
	...
	for (InstructionHandle i = insertAllocPos; i != null; i = i.getNext()) {
		...
		if (target.getName().equals("sync")
		    ...
		    if (cl != null
			    && cl.getClassName().equals(
				    "ibis.satin.SatinObject")) {
	...
	try {
	     ...
	     * 
	     * for (int i = 0; i < params.length; i++) { String paramName =
	     * MethodTable.getParamName(m, i);
	     * 
	     * out.println("        result." +
	     * MethodTable.generatedLocalName(params[i], paramName) + " = " +
	     * MethodTable.generatedLocalName(params[i], paramName) + ";"); }
	...
	try {
		...
		for (int i = 0; i < params.length; i++) {
			...
			if (!isSharedObject(params[i])) {
			    ...
			    out.println("        w.satin_so_reference_param"
				    + i + " = null;");
			}
		    }
		}
	    ...
	    out.println("        } catch (Throwable e) {\n"
		    + "            if (ibis.satin.impl.Config.inletLogger.isDebugEnabled()) {\n"
		    + "                ibis.satin.impl.Config.inletLogger.debug("
		    + "\"caught exception in runremote: " + "\" + e, e);\n"
		    + "            }\n");
		...
		if (isSharedObject(params[i])) {
		     ...
		     * ", owner);");
		     */
		    out.println("        ibis.satin.impl.Satin.getSatin().so.setSOReference(satin_so_reference_param"
			    + i + ", owner);");
		}
	    }
	    out.println("    }\n");
	    ...
	    String guardSignature = signature.substring(0,
		    signature.indexOf(")") + 1)
		    + "Z";
	...
	newMethodInsList.append(ins_f.createInvoke("ibis.satin.impl.Satin",
		"getSatin", new ObjectType("ibis.satin.impl.Satin"),
	...
	newMethodInsList.append(ins_f.createInvoke(
		SOInvocationRecordName(m, clnam, pnam), "<init>", Type.VOID,
		objectId_and_arguments, Constants.INVOKESPECIAL));
	newMethodInsList.append(ins_f.createInvoke("ibis.satin.impl.Satin",
		"broadcastSOInvocation", Type.VOID,
	...
	newMethodInsList.append(ins_f.createInvoke(className,
		"so_local_" + m.getName(), returnType, arguments,
		Constants.INVOKEVIRTUAL));
	// exit the monitor
	...
	if (packageName != null && !packageName.equals("")) {
	    classNameNoPackage = this.className.substring(
