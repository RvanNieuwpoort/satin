...
final class FTCommunication implements Config, ReceivePortConnectUpcall,
	...
	try {
	    String election = "satin "
		    + Victim.clusterOf(s.comm.ibis.identifier())
		    + " cluster coordinator";
	    if (s.ft.clusterCoordinatorIdent == null) {
		// Note: if the satin.masterHost property is set, the code below
		// only
		...
		ftLogger.info("cluster coordinator for cluster "
			+ Victim.clusterOf(s.comm.ibis.identifier()) + " is "
	...
	if (ftLogger.isInfoEnabled()) {
	    ftLogger.info(
		    "SATIN '" + s.ident + "': got lostConnection upcall: "
	...
	if (corpse.equals(s.ident)) {
	    ftLogger.error("SATIN '" + s.ident
		    + "': the registry thinks I have crashed! Exiting.");
	    System.exit(1);
	}
	left(corpse);
	handleCrash(corpse);
	...
	for (int i = 0; i < joiners.length; i++) {
		...
		try {
		    ...
		    System.out
			    .println("Exception while sending CHECKPOINT_INFO to "
				    + joiner + ": " + e);
		}
	    }
		...
		try {
		    ...
		    System.out.println(s.ident
			    + " failed to send COORDINATOR_INFO to " + joiner
			    + ": " + e);
		}
	    }
	...
	try {
		...
		if (ASSERTS && value == null) {
			...
			&& value.type == GlobalResultTableValue.TYPE_POINTER) {
	...
	try {
	    ...
	    Map<Stamp, GlobalResultTableValue> results = (Map<Stamp, GlobalResultTableValue>) m
		    .readObject();
	    synchronized (s) {
