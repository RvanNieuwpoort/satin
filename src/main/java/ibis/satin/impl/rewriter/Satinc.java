/* $Id: Satinc.java 15396 2017-02-01 16:29:04Z rob $ */
...
public final class Satinc extends IbiscComponent {
    ...
    Instruction getSatin(InstructionFactory insf) {
        return insf.createInvoke("ibis.satin.impl.Satin", "getSatin", satinType, Type.NO_ARGS, Constants.INVOKESTATIC);
    }
    ...
    void generateMain(ClassGen clg, Method origMain) {
        ...
        MethodGen new_main = new MethodGen(Constants.ACC_STATIC | Constants.ACC_PUBLIC, Type.VOID, new Type[] { new ArrayType(Type.STRING, 1) },
                new String[] { "argv" }, "main", clg.getClassName(), il, clg.getConstantPool());

        il.append(ins_f.createNew(satinType));
        il.append(new DUP());
        il.append(ins_f.createInvoke("ibis.satin.impl.Satin", "<init>", Type.VOID, Type.NO_ARGS, Constants.INVOKESPECIAL));
        il.append(ins_f.createInvoke("ibis.satin.impl.Satin", "isMaster", Type.BOOLEAN, Type.NO_ARGS, Constants.INVOKEVIRTUAL));
        ...
        il.append(ins_f.createInvoke("ibis.satin.impl.Satin", "client", Type.VOID, Type.NO_ARGS, Constants.INVOKEVIRTUAL));
        ...
        il.append(ins_f.createInvoke("ibis.satin.impl.Satin", "exit", Type.VOID, Type.NO_ARGS, Constants.INVOKEVIRTUAL));
        ...
        new_main.addLocalVariable("argv", new ArrayType(Type.STRING, 1), 0, origMain_handle, null);
    ...
    String invocationRecordName(Method m, String clnam, String pnam) {
        if (pnam != null && !pnam.equals("")) {
            return pnam + ".Satin_" + clnam + "_" + do_mangle(m) + "_InvocationRecord";
        }
    ...
    String localRecordName(Method m) {
        if (packageName != null && !packageName.equals("")) {
            return packageName + ".Satin_" + classNameNoPackage + "_" + do_mangle(m) + "_LocalRecord";
        }
        return "Satin_" + classNameNoPackage + "_" + do_mangle(m) + "_LocalRecord";
    }
    ...
    String returnRecordName(Method m, String clnam, String pnam) {
        if (pnam != null && !pnam.equals("")) {
            return pnam + ".Satin_" + clnam + "_" + do_mangle(m) + "_ReturnRecord";
        }
    ...
    void rewriteAbort(MethodGen m, InstructionList il, InstructionHandle i, int maxLocals) {
        ...
        Instruction ab = ins_f.createInvoke("ibis.satin.impl.Satin", "abort", Type.VOID, new Type[] { irType, irType }, Constants.INVOKEVIRTUAL);
        if (mtab.isClone(m)) {
            int parentPos = 3;
    ...
    void rewriteSync(MethodGen m, InstructionList il, InstructionHandle i, int maxLocals) {
        ...
        il.insert(pos, ins_f.createFieldAccess("ibis.satin.impl.spawnSync.InvocationRecord", "cacheNext", irType, Constants.GETFIELD));
        ...
        for (int k = 0; k < idTable.size(); k++) {
            String invClass = invocationRecordName(getStoreTarget(k), getStoreClass(k), getStorePackage(k));
            ...
            if (idTable.size() > 1) {
                ...
                il.insert(pos, ins_f.createFieldAccess("ibis.satin.impl.spawnSync.InvocationRecord", "storeId", Type.INT, Constants.GETFIELD));
            ...
            if (storeIns != null) {
                ...
                if (isArrayStore(storeIns.getStart())) {
                    ...
                    il.insert(pos, ins_f.createFieldAccess(invClass, "array", new ArrayType(target_returntype, 1), Constants.GETFIELD));

                    il.insert(pos, new ALOAD(maxLocals + 3));
                    il.insert(pos, ins_f.createFieldAccess(invClass, "index", Type.INT, Constants.GETFIELD));
                    il.insert(pos, new ALOAD(maxLocals + 3));
                    ...
                    il.insert(pos, new DUP());
                    il.insert(pos, new ASTORE(maxLocals + 3));
                    il.insert(pos, ins_f.createFieldAccess(invClass, "ref", Type.OBJECT, Constants.GETFIELD));
                    il.insert(pos, ins_f.createCheckCast(pf.getReferenceType(cpg)));
                    il.insert(pos, new ALOAD(maxLocals + 3));
                }
                il.insert(pos, ins_f.createFieldAccess(invClass, "result", target_returntype, Constants.GETFIELD));
    ...
    void insertAbortedCheck(MethodGen m, InstructionList il, InstructionHandle pos) {
        ...
        il.insert(abo, ins_f.createInvoke("ibis.satin.impl.Satin", "getParent", irType, Type.NO_ARGS, Constants.INVOKEVIRTUAL));
        ...
        il.insert(abo, ins_f.createFieldAccess("ibis.satin.impl.spawnSync.InvocationRecord", "aborted", Type.BOOLEAN, Constants.GETFIELD));
    ...
    InstructionHandle insertThrowAbort(MethodGen m, InstructionList il, InstructionHandle pos) {
        InstructionHandle retval;

        retval = il.insert(pos, ins_f.createNew("ibis.satin.impl.aborts.AbortException"));
        il.insert(pos, new DUP());
        il.insert(pos, ins_f.createInvoke("ibis.satin.impl.aborts.AbortException", "<init>", Type.VOID, Type.NO_ARGS, Constants.INVOKESPECIAL));
    ...
    InstructionList getAndRemoveStoreIns(InstructionList il, InstructionHandle i) {
        ...
        do {
            ...
            int inc = i.getInstruction().produceStack(cpg) - i.getInstruction().consumeStack(cpg);
            netto_stack_inc += inc;
            i = i.getNext();
    ...
    void rewriteSpawn(MethodGen m, InstructionList il, Method target, InstructionHandle i, int maxLocals, int spawnId, JavaClass cl) {
        ...
        if (!returnType.equals(Type.VOID)) {
            ...
            if (store instanceof ReturnInstruction) {
                ...
                System.err.println("Error: \"return <spawnable method>\" is not " + "allowed, class " + m.getClassName() + ", line " + l);
                System.exit(1);
            }
            if (store instanceof POP || store instanceof POP2) {
                LineNumberTable t = m.getLineNumberTable(cpg);
        ...
        ih = il.append(ih, ins_f.createInvoke(invocationRecordName(target, clname, pnam), methodName,
                new ObjectType(invocationRecordName(target, clname, pnam)), parameters, Constants.INVOKESTATIC));
        ...
        ih = il.append(ih, ins_f.createInvoke("ibis.satin.impl.Satin", "spawn", Type.VOID, new Type[] { irType }, Constants.INVOKEVIRTUAL));
        if (verbose) {
    ...
    void rewriteMethod(Method mOrig, MethodGen m) {
        ...
        m.addLocalVariable("local_record", new ObjectType("ibis.satin.impl.aborts.LocalRecord"), maxLocals + 3, lr_h, null);
        ...
        for (InstructionHandle i = insertAllocPos; i != null; i = i.getNext()) {
            if (i.getInstruction() instanceof ReturnInstruction) {
                ...
                if (target.getName().equals("sync") && target.getSignature().equals("()V")) {
    ...
    InstructionHandle pushParams(InstructionList il, Method m) {
        ...
        for (int i = 0, param = 0; i < params.length; i++, param++) {
            if (params[i].equals(Type.BOOLEAN) || params[i].equals(Type.BYTE) || params[i].equals(Type.SHORT) || params[i].equals(Type.CHAR)
    ...
    InstructionHandle rewriteStore(MethodGen m, InstructionList il, InstructionHandle i, int maxLocals, String localClassName) {
        ...
        i = il.insert(i, ins_f.createFieldAccess(localClassName, fieldName, type, Constants.PUTFIELD));
        return i;
    }
    ...
    InstructionHandle rewriteLoad(MethodGen m, InstructionList il, InstructionHandle i, int maxLocals, String localClassName) {
        ...
        i.setInstruction(new ALOAD(maxLocals));
        i = i.getNext();
        i = il.insert(i, ins_f.createFieldAccess(localClassName, fieldName, type, Constants.GETFIELD));
    ...
    void generateExceptionHandlingClone(Method mOrig) {
        ...
        m.setArgumentTypes(new Type[] { Type.INT, new ObjectType(localRecordName(mOrig)), new ObjectType("java.lang.Throwable"), irType });

        m.setName("exceptionHandlingClone_" + mOrig.getName());
        m.setAccessFlags(Constants.ACC_PUBLIC | (m.getAccessFlags() & ~(Constants.ACC_PRIVATE | Constants.ACC_PROTECTED)));
        ...
        // Save record
        startLocalPos.setInstruction(new ASTORE(getMaxLocals(m) - 5 + localsShift));
        startLocalPos = startLocalPos.getNext();
    ...
    void rewriteInletMethod(Method mOrig, MethodGen m) {
        ...
        il.insert(pos, ins_f.createInvoke(localClassName, "getNew", local_record_type, paramtypes, Constants.INVOKESTATIC));
    ...
    void regenerateLocalRecord() {
        ...
        for (int i = 0; i < methods.length; i++) {
            ...
            if (!mtab.isClone(m) && mtab.containsInlet(m)) {
                ...
                Method exceptionHandler = recgen.containsMethod("handleException",
                        "(ILjava/lang/Throwable;" + "Libis/satin/impl/spawnSync/InvocationRecord;)V");
                ...
                if (!clone.isStatic()) {
                    ...
                    il.append(insf.createFieldAccess(local_record_name, thisFieldName, thisType, Constants.GETFIELD));
                }
                ...
                il.append(insf.createInvoke(className, clone.getName(), clone.getReturnType(), clone.getArgumentTypes(),
    ...
    void writeLocalRecord(Method m) throws IOException {
        ...
        try {
            ...
            out.println("public final class " + name + " extends ibis.satin.impl.aborts.LocalRecord {");
            ...
            out.println("    public void handleException(int spawnId, " + "Throwable t, ibis.satin.impl.spawnSync.InvocationRecord parent) "
                    + "throws Throwable {");
            out.println("        if (ibis.satin.impl.Config.inletLogger.isDebugEnabled()) {");
            out.println("            ibis.satin.impl.Config.inletLogger.debug(" + "\"handleE: spawnId = \" + spawnId + "
                    + "\", t = \" + t + \", parent = \" + parent + \", " + "this = \" + this);");
            out.println("        }");
    ...
    private boolean isSharedObject(Type t) {
        if (t instanceof ObjectType) {
            ...
            try {
                if (Repository.instanceOf(typ.getClassName(), "ibis.satin.SharedObject")) {
                    return true;
                }
            } catch (ClassNotFoundException e) {
                throw new Error(e);
        ...
        try {
            ...
            for (int i = 0; i < params.length; i++) {
                if (params[i] instanceof ObjectType) {
                    ...
                    if (!implementationOf(clnam, "java.io.Serializable")) {
                        System.err.println(clname + ": parameter of spawnable method " + m.getName() + " with non-serializable type " + clnam);
                        System.err.println(clname + ": all parameters of a spawnable method must" + " be serializable.");
                        errors = true;
                    }
                }
            ...
            if (returnType instanceof ObjectType) {
                ...
                if (!implementationOf(onam, "java.io.Serializable")) {
                    System.err.println(clname + ": spawnable method " + m.getName() + " has non-serializable return type: " + onam + ".");
                    System.err.println(clname + ": the return type of a spawnable method must be " + "serializable.");
                    errors = true;
                }
            }
            ...
            out.println("ibis.satin.impl.spawnSync.SpawnCounter s, " + "ibis.satin.impl.spawnSync.InvocationRecord next, int storeId, "
            ...
            for (int i = 0; i < params_types_as_names.length; i++) {
                if (!isSharedObject(params[i])) {
                    ...
                    out.println("        this.satin_so_reference_param" + i + " = param" + i + ".getObjectIdAndSetNonlocal();");
                }
            }
            ...
            if (invocationRecordCache) {
                ...
                for (int i = 0; i < params_types_as_names.length; i++) {
                    if (!isSharedObject(params[i])) {
                        ...
                        out.println("        res.satin_so_reference_param" + i + " = param" + i + ".getObjectIdAndSetNonlocal();");
                    }
                }
                ...
                for (int i = 0; i < params.length; i++) {
                    if (params[i] instanceof ReferenceType) {
                        if (!isSharedObject(params[i])) {
                            ...
                            out.println("        w.satin_so_reference_param" + i + " = null;");
                        }
                    }
                }
            ...
            out.println("            } catch (Throwable e) {\n" + "                if (ibis.satin.impl.Config.inletLogger.isDebugEnabled()) {\n"
                    + "                    ibis.satin.impl.Config.inletLogger.debug(" + "\"caught exception in runlocal: " + "\" + e, e);\n"
                    + "                }\n" + "                if (! (e instanceof ibis.satin.impl.aborts.AbortException)) {\n"
                    + "                    eek = e;\n" + "                } else if (ibis.satin.impl.Config.abortLogger.isDebugEnabled()) {\n"
                    + "                    ibis.satin.impl.Config.abortLogger.debug(" + "\"got AbortException \" + e, e);\n" + "                }\n"
                    + "            }\n" + "            if (eek != null && !inletExecuted) {\n"
                    + "                if (ibis.satin.impl.Config.inletLogger.isDebugEnabled()) {\n"
                    + "                    ibis.satin.impl.Config.inletLogger.debug(" + "\"runlocal: calling inlet for: " + "\" + this);\n"
            ...
            // runRemote method
            out.println("    public ibis.satin.impl.spawnSync.ReturnRecord " + "runRemote() {");
            out.println("        try {");
            out.print("            ");
            if (!returnType.equals(Type.VOID)) {
                out.print("result = ");
            ...
            out.println("                    ibis.satin.impl.Config.abortLogger.debug(" + "\"got AbortException \" + e, e);");
            out.println("                }");
            out.println("        }");
            ...
            if (returnType.equals(Type.VOID)) {
                out.println("    public ibis.satin.impl.spawnSync.ReturnRecord " + "getReturnRecord() {");
                out.println("        return new " + returnRecordName(m, clname, pnam) + "(null, getStamp());");
                out.println("    }\n");
            } else {
                out.println("    public ibis.satin.impl.spawnSync.ReturnRecord " + "getReturnRecord() {");
                out.println("        return new " + returnRecordName(m, clname, pnam) + "(result, null, getStamp());");
                out.println("    }\n");
            }

            // setSOReferences method
            out.println("    public void setSOReferences()");
            out.println("        throws ibis.satin.impl.sharedObjects.SOReferenceSourceCrashedException {");
            for (int i = 0; i < params.length; i++) {
                if (isSharedObject(params[i])) {
                    /*
                     * out.println("        param" + i + " = " + "(" +
            ...
            out.println("        java.util.Vector<String> refs = new java.util.Vector<String>();");
            for (int i = 0; i < params.length; i++) {
                if (isSharedObject(params[i])) {
            ...
            String guardSignature = signature.substring(0, signature.indexOf(")") + 1) + "Z";
            String guardName = "guard_" + m.getName();

            if (gen_c.containsMethod(guardName, guardSignature) != null) {
                if (verbose) {
                    System.out.println("Generating a guard function for " + m.getName());
                }
            ...
            // method to clear object params to allow th e GC to clean them up
            // --Rob
    ...
    void writeReturnRecord(Method m, String clname, String pnam) throws IOException {
        ...
        try {
            ...
            out.println("public final class " + name + " extends ibis.satin.impl.spawnSync.ReturnRecord {");
            if (!returnType.equals(Type.VOID)) {
                out.println("    " + returnType + " result;\n");
            }
            ...
            out.print("    public " + name + "(");
            if (!returnType.equals(Type.VOID)) {
                out.println(returnType + " result, Throwable eek, ibis.satin.impl.spawnSync.Stamp stamp) {");
            } else {
                out.println(" Throwable eek, ibis.satin.impl.spawnSync.Stamp stamp) {");
            }
            ...
            out.println("    public void assignTo(ibis.satin.impl.spawnSync.InvocationRecord rin) {");
            out.println("        " + invocationRecordName(m, clname, pnam) + " r = (" + invocationRecordName(m, clname, pnam) + ") rin;");
    ...
    String SOInvocationRecordName(Method m, String clnam, String pnam) {
        if (pnam != null && !pnam.equals("")) {
            return pnam + ".Satin_" + clnam + "_" + do_mangle(m) + "_SOInvocationRecord";
        }
        return "Satin_" + clnam + "_" + do_mangle(m) + "_SOInvocationRecord";
    }

    String SOInvocationRecordFileBase(Method m, String clnam, String pnam) {
        return getFileBase(pnam, clnam + "_" + do_mangle(m), "Satin_", "_SOInvocationRecord");
    }
    ...
    private void writeSOInvocationRecord(Method m, String clname, String pnam) throws java.io.IOException {
        ...
        if (verbose) {
            System.err.println("Generating inv rec for method: " + m.getName() + " with signature: " + m.getSignature());
        }
        ...
        for (i = 0; i < params.length; i++) {
            if (params[i] instanceof ObjectType) {
                String clnam = ((ObjectType) params[i]).getClassName();
                if (!implementationOf(clnam, "java.io.Serializable")) {
                    System.err.println(clname + ": write method" + " with non-serializable parameter type " + clnam);
                    System.err.println(clname + ": all parameters of a write method" + " must be serializable.");
                    out.close();
                    System.exit(1);
                }
            }
        ...
        out.println("public final class " + name + " extends ibis.satin.impl.sharedObjects.SOInvocationRecord {");
    ...
    void rewriteSOMethod(Method m, String clnam, String pnam) {
        ...
        if (verbose) {
            System.err.println("Rewriting method: " + m.getName() + " with signature: " + m.getSignature());
        }
        ...
        // Satin.getSatin().
        // broadcastSOInvocation(new
        // Satin_methodname_SOInvocationRecord(params))
    ...
    void doSORewrite() throws IOException {
        ...
        gen_c.addField(new FieldGen(Constants.ACC_STATIC, Type.BOOLEAN, "$SOrewritten$", cpg).getField());

        // add things to the constant pool
        /*
         * int getSatin = cpg.addMethodref("ibis.satin.impl.Satin", "getSatin",
         * "()Libis/satin/impl/Satin"); int broadcastSOInvocation =
         * cpg.addMethodref("ibis.satin.impl.Satin", "broadcastSOInvocation",
        ...
        for (int j = 0; j < methods.length; j++) {
            ...
            if (a.isSpecial(method)) {
                ...
                toCompiler.add(SOInvocationRecordFileBase(method, classNameNoPackage, packageName) + ".java");
            }
        }
        if (toCompiler.size() > 0) {
            compile(toCompiler, className);
