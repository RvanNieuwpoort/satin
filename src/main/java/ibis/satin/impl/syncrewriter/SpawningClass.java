...
class SpawningClass extends ClassGen {
    ...
    SpawningClass(JavaClass javaClass, SpawnSignature[] allSpawnSignatures,
	    Debug d) throws NoSpawningClassException, AssumptionFailure {
	super(javaClass);
	...
	if (spawnSignatures.length == 0) {
		...
		System.out.println("All spawning methods of class "
			+ getClassName() + " already sync.");
	    }
	    return false;
	}
	...
	if (throwRewriteFailure)
	    throw new ClassRewriteFailure();
	return true;
    }
	...
	for (Method method : methods) {
	    try {
		SpawningMethod spawningMethod = new SpawningMethod(method,
			getClassName(), getConstantPool(), spawnSignature, 0,
     ...
     */
    private void rewriteForSpawnSignature(SpawnSignature spawnSignature,
	    Analyzer analyzer) throws MethodRewriteFailure {
	boolean throwRewriteFailure = false;

	d.log(0, "rewriting for %s\n", spawnSignature);

	int indexSync = getConstantPool().addMethodref(
		spawnSignature.getClassName(), "sync", "()V");
	...
	for (Method method : methods) {
	    try {
		SpawningMethod spawningMethod = new SpawningMethod(method,
			getClassName(), getConstantPool(), spawnSignature,
			indexSync, new Debug(d.turnedOn(),
				d.getStartLevel() + 2));
		d.log(1, "%s calls %s\n", method.getName(), spawnSignature);
		...
		System.out.println("Method " + method.getName() + " in class "
			+ getClassName() + " already contains a sync call");
	    } catch (NoSpawningMethodException e) {
		d.log(1, "%s doesn't call %s\n", method.getName(),
			spawnSignature);
		// spawnSignature is not called
	...
	if (throwRewriteFailure)
	    throw new MethodRewriteFailure();
	d.log(0, "rewritten for %s\n", spawnSignature);
    }

    /*
     * it is true that creating a spawning method happens twice, once to find
     * the spawnsignatures for this class and once for rewriting the methods. It
     * seems a better idea to just store spawningMethods in the beginning and
     * rewrite that. However, every time you rewrite a spawningMethod for
     * 'spawnSignature1' and then you would have to update all these
     * spawningMethods for the method that has been rewritten, in case you are
     * going to rewrite for 'spawnSignature2'. Rewriting is an iterative
     * process:
     * 
     * spawningMetod = new SpawningMethod(method) this.remove(method)
     ...
     * this.remove(method) spawningMethod.rewriteFor(spawnSignature1);
     * this.add(spawningMethod.getMethod());
     * 
     ...
     */
    private void getSpawnSignatures(ArrayList<SpawnSignature> spawnSignatures,
	    SpawnSignature spawnSignature) throws AssumptionFailure {
	Method[] methods = getMethods();
    ...
    private SpawnSignature[] getSpawnSignatures(
	    SpawnSignature[] allSpawnSignatures) throws AssumptionFailure {
