...
class LoadAwareBasicBlock extends BasicBlock {
	...
	for (InstructionContext ic : instructions) {
	    ...
	    LocalVariableGen l = methodGen.findLocalVar(current, lg.getIndex(),
		    false);
    ...
    boolean noAliasesLoadWithIndexBefore(InstructionHandle ih,
	    LocalVariableGen lg) {
	for (InstructionContext ic : instructions) {
	    InstructionHandle current = ic.getInstruction();
	    if (current.equals(ih)) {
		...
		if (!methodGen.isUsedForArrayLoad(current)
			&& !methodGen.isUsedForArrayLength(current)
			&& !methodGen.isUsedForArrayStore(current)
			&& !methodGen.isUsedForPutField(current)
    ...
    /**
     * Checks that any store in this basic block to the specified variable is
     * the result of a new() or a null.
     * 
     * @param ih
     *            handle up to where to investigate.
     * @param localVarIndex
	...
	for (InstructionContext ic : instructions) {
		...
		LocalVariableGen l1 = methodGen.findLocalVar(current,
			lg.getIndex(), false);
		if (l1 != lg) {
		    prev = current;
		    continue;
		...
		if (prev != null) {
			...
			if (invoker.getMethodName(methodGen.getConstantPool())
				.equals("<init>")
		    ...
		    if (i instanceof NEWARRAY || i instanceof ANEWARRAY
	...
	ReferenceType cls = invoker.getReferenceType(methodGen
		.getConstantPool());
	...
	return nonEscapingConstructor(javaClass,
		invoker.getSignature(methodGen.getConstantPool()));
    }

    boolean nonEscapingConstructor(JavaClass javaClass,
	...
	for (Method method : methods) {
	    if (method.getName().equals("<init>")
		    && method.getSignature().equals(constructorSignature)) {
		if (nonEscapingConstructor(superClass, "()V")) {
		    ClassGen cg = new ClassGen(javaClass);
		    MethodGen mg = new MethodGen(method,
			    javaClass.getClassName(), cg.getConstantPool());
		    // Now check all ALOAD 0 instructions. They may only be used
		    // for
			...
			if (i instanceof ALOAD && ((ALOAD) i).getIndex() == 0) {
				...
				// Find instructions that consume exactly this
				// load (but not more,
				// otherwise it could be a parameter to another
				// constructor).
				InstructionHandle[] users = mg
					.findExactInstructionConsumers(h);
				if (users.length != 1) {
				    return false;
				}
				...
				if (i instanceof INVOKESPECIAL) {
				    INVOKESPECIAL invoker = (INVOKESPECIAL) i;
				    if (!invoker.getMethodName(
					    mg.getConstantPool()).equals(
					    "<init>")
					    || !nonEscapingConstructor(
						    superClass,
						    invoker.getSignature(mg
							    .getConstantPool()))) {
					return false;
				    }
				} else {
    ...
    private boolean containsLoadWithIndexAfter(InstructionHandle ih,
	    boolean ignoreInstructions, LocalVariableGen lg) {
	InstructionHandle start = lg.getStart();
	InstructionHandle prev = start.getPrev();
	// The initial store is not included in the start/end range, so include
	// the previous
	// instruction if it exists.
	InstructionHandle end = lg.getEnd();
	int startPosition = prev != null ? prev.getPosition() : start
		.getPosition();
	...
	for (InstructionContext ic : instructions) {
	    ...
	    } else if (current.getPosition() >= startPosition
		    && current.getPosition() <= endPosition
    ...
    private boolean noAliasesLoadWithIndex(InstructionHandle ih,
	    boolean ignoreInstructions, LocalVariableGen lg) {
	InstructionHandle start = lg.getStart();
	InstructionHandle prev = start.getPrev();
	// The initial store is not included in the start/end range, so include
