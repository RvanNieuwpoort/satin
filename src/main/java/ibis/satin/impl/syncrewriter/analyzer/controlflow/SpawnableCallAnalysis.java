...
public class SpawnableCallAnalysis {
    ...
    /**
     * Instantiates a spawnable call analysys from a spawnable call and a graph
     * of basic blocks.
     * 
     * @param spawnableCall
     *            The spawnable call to be analyzed.
     * @param basicBlockGraph
     *            The basic block graph that helps analyzing.
     * @param d
     *            A debug instance to write debug information to.
     */
    public SpawnableCallAnalysis(SpawnableCall spawnableCall,
	    BasicBlockGraph basicBlockGraph, Debug d) {
	this.d = d;

	int idBasicBlock = basicBlockGraph.getIdBasicBlock(spawnableCall
	...
	InstructionHandle[] stackConsumers = mg
		.findInstructionConsumers(invoker);
	...
	for (InstructionHandle consumer : stackConsumers) {
	    ...
	    // Fairly cheap check could be: the object is local, allocated in
	    // the
	    // spawning method, and no alias is done. This may, however, be a
	    // bit
	    // too restrictive. On the other hand, allowing for more makes the
	    // check
	    // much more difficult. --Ceriel
	    if (indexToCheck >= 0) {
		LocalVariableGen lg = mg.findLocalVar(invoker, indexToCheck);
		// So, here we have a spawn that stores into an object field or
		// an
		// array element.
		d.log(0,
			"spawn stores in array element or object field, index "
				+ indexToCheck);
		if (mg.isParameter(indexToCheck)) {
		    // The object is indicated by a parameter. We give up.
		    d.warning("The result of the spawn at line "
			    + spawnLineno
			    + " of class "
			    + className
		...
		// Here, the object is indicated by a local variable. This is OK
		// if
		// 1. There is no load of this local variable present in front
		// of the spawn.
		// 2. The object is allocated in the spawning method, and the
		...
		for (BasicBlock b : predecessors) {
		    ...
		    if (!lb.noAliasesLoadWithIndex(lg)
			    || !lb.noAliasesStoreWithIndex(lg)) {
			d.warning("The result of the spawn at line "
				+ spawnLineno
				+ " of class "
				+ className
		...
		boolean noAliasLoad = lb.noAliasesLoadWithIndexBefore(invoker,
			lg);
		boolean noaliasStore = noAliasLoad ? lb
			.noAliasesStoreWithIndexBefore(invoker, lg) : false;
	...
	if (resultMayHaveAliases) {
	    ...
	    storeLoadPaths.add(new StoreLoadPath(spawnableCall
		    .getInvokeInstruction(), callerBlock, spawnableCall
		    .getIndicesStores()));
	}
	...
	if (storeLoadPaths.size() == 0) {
	    ...
	    latestCommonSubPath = Path.getLatestCommonSubPath(endingPaths
		    .toArray(endingPathsArray));
	} else {
	    Path[] storeLoadPathsArray = new Path[storeLoadPaths.size()];
	    latestCommonSubPath = Path.getLatestCommonSubPath(storeLoadPaths
		    .toArray(storeLoadPathsArray));
	}
	/*
	...
	if (mg.isArrayStore(ih.getInstruction())
    ...
    private void fillStoreLoadPaths(ArrayList<StoreLoadPath> storeLoadPaths,
	    Path path, SpawnableCall spawnableCall) {
	LocalVariableGen[] indicesStores = spawnableCall.getIndicesStores();
	InstructionHandle invokeInstruction = spawnableCall
		.getInvokeInstruction();
	...
	if (!spawnableCall.exceptionsHandled()) {
	    ...
	    try {
		storeLoadPath = new StoreLoadPath(invokeInstruction, path,
			indicesStores);
	    } catch (NeverReadException e) {
	    }
	    if (storeLoadPath != null
		    && !storeLoadPaths.contains(storeLoadPath)) {
		storeLoadPaths.add(storeLoadPath);
	    }
	}
    }

    private ArrayList<StoreLoadPath> getStoreLoadPaths(
	    ArrayList<Path> endingPaths, SpawnableCall spawnableCall) {
