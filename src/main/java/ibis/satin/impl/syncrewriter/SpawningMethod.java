...
public class SpawningMethod extends MethodGen {
    ...
    SpawningMethod(Method method, String className,
	    ConstantPoolGen constantPoolGen, SpawnSignature spawnSignature,
	    int indexSync, Debug d) throws NoSpawningMethodException,
	    AssumptionFailure {
	...
	MethodGen spawnSignatureGen = new MethodGen(spawnSignature.getMethod(),
		spawnSignature.getClassName(), constantPoolGen);

	ArrayList<SpawnableCall> spawnableCalls = getSpawnableCalls(
		constantPoolGen, spawnSignatureGen);
	...
	InstructionHandle[] ihs = analyzer.proposeSyncInsertion(this,
	...
	for (InstructionHandle ih : ihs) {
	    ...
	    System.out.println("Insert a sync() in method " + getName()
		    + " at line " + l + " in class " + getClassName());
	}
    }
	...
	for (InstructionHandle ih : ihs) {
	    InstructionHandle syncInvoke = instructionList.insert(ih,
		    new INVOKEVIRTUAL(indexSync));
	    InstructionHandle newTarget = instructionList
		    .insert(syncInvoke, spawnableCalls.get(0)
			    .getObjectReference().getInstruction());
	...
	String className = (cls instanceof ObjectType) ? ((ObjectType) cls)
	...
	for (ObjectType i : types) {
	    try {
		if (i.equals(type) || type.subclassOf(i))
		    return true;
	    } catch (ClassNotFoundException e) {
		continue;
	    }
    ...
    private boolean hasRightType(CodeExceptionGen codeException,
	    MethodGen method) {
    ...
    private ArrayList<CodeExceptionGen> getExceptionHandlers(
	    InstructionHandle ih, MethodGen spawnSignature) {
	...
	for (CodeExceptionGen codeException : codeExceptions) {
	    ...
	    /*
	     * if (codeException.containsTarget(ih) &&
	     * hasRightType(codeException, spawnSignature)) {
	     * result.add(codeException); }
	     */
	    if (Util.containsTarget(codeException, ih)
		    && hasRightType(codeException, spawnSignature)) {
		result.add(codeException);
	    }
	}
	...
	for (InstructionHandle current = start.getNext(); current != end
		.getNext(); current = current.getNext()) {
	    try {
		...
		for (LocalVariableGen ll : this.getLocalVariables()) {
		    if (ll != l && l.getIndex() == ll.getIndex()
			    && l.getName().equals(ll.getName())) {
    ...
    private SpawnableCall getSpawnableCallWithException(
    ...
    private SpawnableCall getSpawnableCallWithException(
	    InstructionHandle invokeInstruction, MethodGen spawnSignatureGen) {
	ArrayList<CodeExceptionGen> exceptionHandlers = getExceptionHandlers(
		invokeInstruction, spawnSignatureGen);
	...
	if (exceptionHandlers.size() == 0) {
	    ...
	    return getSpawnableCallWithException(invokeInstruction,
		    exceptionHandlers);
	}
    }
     ...
     */
    private LocalVariableGen[] findIndexStore(
	    InstructionHandle spawnableMethodInvoke) throws ResultNotStored {
	...
	try {
	    ...
	    return resultIndices.toArray(new LocalVariableGen[resultIndices
		    .size()]);
	} catch (ClassCastException e) {
	    throw new ResultNotStored();
	}
    ...
    private SpawnableCall getSpawnableCallReturningValue(
	    InstructionHandle invokeInstruction) {
	try {
	    return new SpawnableCall(invokeInstruction,
		    getObjectReferenceLoadInstruction(invokeInstruction),
    ...
    private ArrayList<SpawnableCall> getSpawnableCalls(
	    ConstantPoolGen constantPoolGen, MethodGen spawnSignatureGen)
	    throws AssumptionFailure {

	ArrayList<SpawnableCall> spawnableCalls = new ArrayList<SpawnableCall>();

	InstructionList il = getInstructionList();
	int indexSpawnableCall = constantPoolGen
		.lookupMethodref(spawnSignatureGen);
	if (indexSpawnableCall < 0)
	    return spawnableCalls;

	INVOKEVIRTUAL spawnableInvoke = new INVOKEVIRTUAL(indexSpawnableCall);
	...
	do {
		...
		if (instruction.produceStack(constantPoolGen) > 0) {
			...
			spawnableCalls.add(getSpawnableCallWithException(ih,
				spawnSignatureGen));
		    } else {
			throw new AssumptionFailure(
				"Not satisfying assumption that spawnable method returns something or throws something");
		    }
		} else {
		    throw new AssumptionFailure(
			    "Not satisfying assumption that spawnable method returns something or throws something");
