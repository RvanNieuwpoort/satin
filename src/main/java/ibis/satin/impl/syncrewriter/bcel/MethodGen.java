...
public class MethodGen extends org.apache.bcel.generic.MethodGen {
     ...
     *            constant pool
     */
    public MethodGen(Method method, String className,
    ...
    /**
     * Returns the instruction handles that consume what instructionHandle ih
     * put onto the stack.
     * 
     ...
     * 
     * @param ih
     *            The instructionHandle that produced something on the stack
     * 
     * @return The instruction handles that consume what ih produced on the
     *         stack.
     */
    public InstructionHandle[] findExactInstructionConsumers(
	...
	ArrayList<InstructionHandle> consumers = getInstructionsConsuming(
		wordsOnStack, ih.getNext(), constantPoolGen, controlFlowGraph,
		EXACT);
	InstructionHandle[] consumersArray = new InstructionHandle[consumers
		.size()];
    ...
    /**
     * Tests whether an object load instruction is used for puting something in
     * a field of an object.
     * 
     * @param loadInstruction
     *            the load instruction that loads the object reference onto the
     *            stack.
     * 
    ...
    /**
     * Tests whether an object load instruction is used for storing something in
     * an array.
     * 
     * @param loadInstruction
     *            the load instruction that loads the object reference onto the
	...
	if (loadInstruction.getInstruction() instanceof ALOAD
		|| loadInstruction.getInstruction() instanceof AALOAD) {
	    InstructionHandle[] loadInstructionConsumers = findExactInstructionConsumers(loadInstruction);
	    if (loadInstructionConsumers.length == 0)
		return false;
     ...
     * be false. It uses {@link #isUsedForArrayStore(InstructionHandle)} and
     * {@link #isUsedForPutField(InstructionHandle)}.
     * 
     * @param ih
     *            The instruction that is tested to load to a local variable
     *            index.
     * @param localVarIndex
     *            The local variable index to which the instruction may load
     * 
	...
	try {
	    ...
	    return loadInstruction.getIndex() == localVarIndex
		    && !isUsedForArrayLength(ih) && !isUsedForArrayStore(ih)
		    && !isUsedForPutField(ih);
	...
	try {
	    StoreInstruction storeInstruction = (StoreInstruction) (ih
		    .getInstruction());
    ...
    /**
     * Tests whether an instruction consumes what another instruction puts onto
     * the stack.
     * 
     * @param consumer
     *            The instruction that consumes what consumee produces
     * @param consumee
     *            The instruction of which the stack production is consumed.
     * @return true if consumer consumes what consumee produces on the stack,
     *         false otherwise.
     */
    public boolean consumes(InstructionHandle consumer,
	    InstructionHandle consumee) {
	ConstantPoolGen constantPoolGen = getConstantPool();
	ControlFlowGraph controlFlowGraph = new ControlFlowGraph(this);
     ...
     *         false otherwise.
     */
    public boolean consumesExtensively(InstructionHandle consumer,
	    InstructionHandle consumee) {
	ConstantPoolGen constantPoolGen = getConstantPool();
	ControlFlowGraph controlFlowGraph = new ControlFlowGraph(this);
	return consumesExtensively(consumer, consumee, constantPoolGen,
		controlFlowGraph);
    }
     ...
     * 
     * It uses
     * {@link #consumesExtensively(InstructionHandle, InstructionHandle)} to
     * find out which instruction consumes another.
     * 
     * There may be cases, for example with complicated DUP structures where the
     * analysis can't find out where object reference is, the method will then
     * throw an error. These cases should be added to the code then...
     * 
     * @param ih
     *            The instructionHandle that invokes something on an object.
     ...
     */
    public InstructionHandle getObjectReferenceLoadInstruction(
	    InstructionHandle ih) {
	...
	throw new Error("Can't find the object reference load instruction for "
		+ ih.getInstruction());
    }
     ...
     *            The instruction that is a store instruction.
     * @return The index of the local variable in which is stored.
     * @throws ClassCastException
     *             The instructionHandle is not a store instruction.
     * @see #getObjectReferenceLoadInstruction(InstructionHandle)
     */
    public LocalVariableGen getIndexStore(InstructionHandle instructionHandle)
	    throws ClassCastException {
	try {
	    StoreInstruction storeInstruction = (StoreInstruction) (instructionHandle
		    .getInstruction());
	...
	for (LocalVariableGen localVar : localVars) {
	    ...
	    if (startScope == startHandler
		    || startScope == startHandler.getNext()
    ...
    private ArrayList<InstructionHandle> getAllObjectLoadInstructions(
	    InstructionList il) {
    ...
    private ArrayList<InstructionHandle> getInstructionsConsuming(int nrWords,
	    InstructionHandle current, ConstantPoolGen constantPoolGen,
	...
	for (InstructionContext successorContext : currentContext
		.getSuccessors()) {
	...
	return instruction instanceof AALOAD || instruction instanceof BALOAD
		|| instruction instanceof CALOAD
		|| instruction instanceof DALOAD
		|| instruction instanceof FALOAD
		|| instruction instanceof IALOAD
		|| instruction instanceof LALOAD
		|| instruction instanceof SALOAD;
    }
    ...
    private InstructionHandle getDUP(
	    ArrayList<InstructionHandle> instructionHandles) {
	if (instructionHandles.size() == 1
		&& instructionHandles.get(0).getInstruction() instanceof DUP) {
	    return instructionHandles.get(0);
	}
	throw new Error(
		"Very strange situation, not handled yet, controlflow in DUP's????\n");
    }

    private boolean containsArrayLoad(
	    ArrayList<InstructionHandle> instructionHandles) {
	for (InstructionHandle ih : instructionHandles) {
    ...
    private InstructionHandle getArrayLoad(
	    ArrayList<InstructionHandle> instructionHandles) {
	if (instructionHandles.size() == 1
	...
	throw new Error(
		"Very strange situation, not handled yet, controlflow in ArrayLoad's????\n");
    }
     ...
     * instruction. We then want to know if the consumer consumes what the DUP
     * produced on the stack.
     */
    private boolean consumesIncludingDUP(InstructionHandle consumer,
	    InstructionHandle consumee, ConstantPoolGen constantPoolGen,
	    ControlFlowGraph controlFlowGraph) {
	int wordsOnStack = consumee.getInstruction().produceStack(
		constantPoolGen);
	ArrayList<InstructionHandle> consumers = getInstructionsConsuming(
		wordsOnStack, consumee.getNext(), constantPoolGen,
		controlFlowGraph, EXACT);
     ...
     */
    private boolean consumesIncludingArrayLoad(InstructionHandle consumer,
	    InstructionHandle consumee, ConstantPoolGen constantPoolGen,
	    ControlFlowGraph controlFlowGraph) {
	int wordsOnStack = consumee.getInstruction().produceStack(
		constantPoolGen);
	...
	return containsArrayLoad(consumers)
		&& consumes(consumer, getArrayLoad(consumers));
    }

    private boolean consumesExtensively(InstructionHandle consumer,
	    InstructionHandle consumee, ConstantPoolGen constantPoolGen,
	    ControlFlowGraph controlFlowGraph) {
	return consumes(consumer, consumee, constantPoolGen, controlFlowGraph)
		|| consumesIncludingArrayLoad(consumer, consumee,
			constantPoolGen, controlFlowGraph)
		|| consumesIncludingDUP(consumer, consumee, constantPoolGen,
			controlFlowGraph);
    }

    private boolean consumes(InstructionHandle consumer,
	    InstructionHandle consumee, ConstantPoolGen constantPoolGen,
	    ControlFlowGraph controlFlowGraph) {
	int wordsOnStack = consumee.getInstruction().produceStack(
		constantPoolGen);
	...
	for (LocalVariableGen l : lt) {
		...
		int startIndex = prev != null ? prev.getPosition() : start
			.getPosition();
		int endIndex = end.getPosition();
		if (ih.getPosition() >= startIndex
			&& ih.getPosition() <= endIndex) {
		    return l;
		}
	    }
	}
