...
class ClassViewer {
	...
	if (completeMessage.length() > NR_CHARS_ON_LINE) {
	    ...
	    print(out,
		    level + 1,
		    completeMessage.substring(NR_CHARS_ON_LINE,
	...
	for (InstructionHandle ih : instructionHandles) {
	    ...
	    out.printf("%d:\t(c: %d, p: %d, t: %d)\t%s\n", ih.getPosition(),
		    consumed, produced, balanceOnStack, ih.getInstruction()
			    .toString(cp));
	}
    }
	...
	for (Method method : methods) {
		...
		out.println((method.getCode()).toString(true));
	    out.println();
	}
    }
